  ...
  for(unsigned int itr=0;itr<num_iteration;++itr) {  // Gauss-Seidel iteration
    for (unsigned int src_i = 1; src_i < src.rows() - 1; ++src_i) {
      for (unsigned int src_j = 1; src_j < src.cols() - 1; ++src_j) {
        ...
        double sum_of_ret_pixels = ret_n + ret_s + ret_e + ret_w;
        double sum_of_src_pixels = src_n + src_s + src_e + src_w;
        double ret_c = (sum_of_ret_pixels + 4 * src_c - sum_of_src_pixels) / 4.0; // change this line
        // no edit below
